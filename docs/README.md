...
* '''`spark-4` (Head Compute Node, current `PRIMARY_HOST`):'''
...
* '''`spark-3` (Worker Compute Node, current `SECONDARY_HOST`):'''
...
'''Host identity is config-derived, not hardcoded.''' `PRIMARY_HOST` / `SECONDARY_HOST` / `PRIMARY_HOST_IP` in `dgx-orchestrator.py` are derived once from the first/second host listed in `cluster_config.yaml`'s `hosts:` block — every code path that used to hardcode `spark-3`/`spark-4`/`10.0.14.43` literally now reads from these instead (see `docs/TOMBSTONES.md` #73). This is what will make it safe to eventually point a second, independent orchestrator instance (`maestro2`, not yet built) at a different host pair — e.g. `spark-5`/`spark-6` on a separate, non-interconnected network segment — without further code changes. `cluster_config.yaml` for the current spark-3/4 pair is left untouched; the pairing constraint itself (a deployed pair must share a fabric) isn't yet expressed anywhere in code — see `ARCHITECTURE-MIGRATION-PLAN.md`'s Phase 3 section.
...
Every deploy binds a per-run, per-host directory (`~/.cache/ray-logs/<deploy_run_id>/<host>`) to each container's `/tmp/ray`, so a crashed worker's Ray session logs — stdout/stderr included — survive container teardown instead of vanishing with it. This is the difference between a crash being diagnosable at all versus permanently a mystery. `dgx-config prune-ray-logs --retention-days N --dry-run` reclaims this space on an age basis (default 7 days, via `cluster_config.yaml`'s `tuning.crash_log_retention_days`) rather than waiting on free-space pressure, since these logs stay small enough to otherwise accumulate indefinitely. `tuning.debug_launch_blocking` (default `false`) sets `CUDA_LAUNCH_BLOCKING=1` for forcing synchronous CUDA kernel launches when actively chasing a live repro — costs real decode throughput, not meant to be left on.

=== 6. Version Tracking & Deploy Confirmation ===
`ORCHESTRATOR_VERSION` in `dgx-orchestrator.py` is a hand-bumped string surfaced in four places: daemon startup logs, every `/api/status` response, the CLI `status` summary line, and a badge in the dashboard header next to Server Time. This exists because "the code was edited but never actually deployed" is a real failure mode here — the deploy workflow (`git push` locally → `git pull` on `maestro` → copy into the running container → restart) has a step where a forgotten `git push` silently leaves the daemon running old code with no error anywhere. Check the version badge (or `orchestrator_version` in `/api/status`) after any deploy of a `dgx-orchestrator.py` change to confirm it actually landed, rather than assuming it did.

=== 7. Status Cache Staleness ===
`/api/status` responses carry `stale` / `stale_for_seconds`, reflecting how long the currently-served status snapshot has actually been in use. A wedged internal computation now surfaces this instead of silently serving a frozen snapshot indefinitely — useful for confirming "is the dashboard actually live right now" without a manual `curl`. Not yet surfaced as a dashboard banner; currently API-only.
...
* '''Inspect Orphaned SysV IPC Segments (read-only):''' `dgx-config ipc-inventory`
* '''Sweep Orphaned IPC Segments:''' `dgx-config sweep-ipc-orphans --dry-run` (drop `--dry-run` to actually remove; only ever removes segments with a kernel-tracked `nattch == 0`)
* '''Reclaim Persisted Crash Log Space:''' `dgx-config prune-ray-logs --retention-days 7 --dry-run` (age-based, not free-space-gated — see "Deployment & Topology Architecture" below for what these logs are)
* '''Repair a Ledger Entry After a Restart-Induced Undercount:''' `dgx-config correct-ledger --dry-run` (auto-detects the currently-serving model/topology/live metrics with zero arguments; refuses to overwrite with smaller values unless `--force`; backs up the ledger file before any write)
