...
### Test harness consolidation (2026-09-10)

| Item | Status | Notes |
|---|---|---|
| `agentic_eval.py` and `h2h_agentic.py` were near-duplicates | **RESOLVED 2026-09-10** | Byte-identical `chat()`; the second was copied from the first a day later, took the harder tasks and dropped the hardening. Merged into `agentic_eval.py`, keeping its plumbing (`--out`, stale-file clear, HTTP-body surfacing, `exit 2`) and the other's tasks. Two bugs fixed on the way in, both of which made results WRONG rather than noisy: `score_bugfix()` exec'd emitted code whose `from utils import clamp` could not resolve, so `functional` was False for every CORRECT answer; and `agentic_speed()` derived tok/s from SSE delta count, undercounting every speculative-decode model by a factor varying with acceptance rate -- #52 reintroduced in a copy. The speed test was dropped rather than fixed; `benchmark.py` already measures decode from `stream_options` usage. |
| `ab_test.py` has no agentic axis | **OPEN** | It compares throughput; nothing in it asks whether the model is any good. `agentic_eval.py` answers that and is cheap because it deploys nothing. Proposed: `ab_test.py --agentic` shells out to it against the endpoint it just deployed, the way the retired `sweep.py` did. Composition, not a merge -- folding it in would make every agentic eval inherit a deploy cycle it does not need, and inherit the reserved-host refusal for a workload that never needed it. **This is the input `MODELS.md`'s empty "relative strengths" section has been waiting for**; the Gemma4 DFlash-vs-MTP split (coding +54%, extraction +178%, prose level) is the argument that one throughput number hides the thing you actually want to know. |
| `smoke_test_mc.py` T9 exercises a removed feature | **OPEN** | Writes a `models.yaml` to a temp dir and sets `USE_LEGACY_CATALOG=1` to test a fallback removed by #112 and cleared from `cache_cluster_assets.py` on 2026-09-09. Either it passes and asserts nothing, or it has been red unnoticed. Same class as `verify_recipe_equivalence.py`; delete the case or banner it. |
