...
* **Bug:** Host nodes configured to auto-start Docker occasionally fell into reboot loops when encountering corrupted runtime configurations.
* **Fix:** Docker and Containerd services were permanently disabled on boot across the cluster. Added remote wakeup injections directly into `dgx-orchestrator.py` to ensure daemons are safely initialized via the SSH multiplexer socket prior to deployment execution.

### 8. Llama-3.3 Single-Node CUDA Graph Kernel Lockup

* **Bug:** Attempting a 1-node deployment of Llama-3.3-70B occasionally froze the primary node's memory bus during PyTorch Inductor autotune graph compilation, causing SSH responsiveness to completely lock.
* **Fix:** Bound the `--enforce-eager` flag strictly to `llama-3.3-70b`'s `1_node` definition in `models.yaml` to permanently bypass CUDA graph generation.
