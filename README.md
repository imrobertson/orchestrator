...
The orchestrator requires a HuggingFace authentication token to pull gated models (like Llama 3) and tokenizer configurations. Create a `.secrets` file in the project root:
...
''Note: The orchestrator's get_hf_token() function will automatically parse this file and securely inject it into the containers at runtime.''
...
* '''Pre-Cache Assets:''' `cache_cluster_assets.py` parses `models.yaml` and parallel-downloads Docker images and HuggingFace safetensors to both DGX nodes (`/home/tetrel/.cache/huggingface`). Run this on `maestro` prior to toggling offline mode.
* '''Offline Mode Toggle:''' Within the Web Dashboard, clicking the network badge toggles the cluster into offline mode, injecting `HF_HUB_OFFLINE=1` and `TRANSFORMERS_OFFLINE=1` into all new container deployments.

=== Performance Benchmarking (MTP-Aware) ===

* '''Streaming Benchmark (`benchmark.py`):''' Accurately times Time-To-First-Token (TTFT) and Decode tokens/second.
* '''MTP Protection:''' Specifically hardened against vLLM Multi-Token Prediction (MTP) stream buffering to prevent mathematical division-by-zero errors.
* '''Ledger Tracking:''' All results, including exact token counts and engine configurations, are automatically appended to `benchmark_ledger.csv`.

== Core Model Catalog (`models.yaml`) Summary ==
''Note: Configurations and image tags inside `models.yaml` are actively informed by upstream GitHub model recipes. We are converging our custom architectures with these upstream project recipes to allow for verbatim adoption in the future.''
